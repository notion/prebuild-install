...
test('custom config and aliases', (t) => {
  const args = [
    '--arch ARCH',
    '--platform PLATFORM',
...
test('npm args are passed on from npm environment into rc', (t) => {
  const env = {
...
test('npm_config_* are passed on from environment into rc', (t) => {
  const env = {
...
test('can pass in external package config to rc', (t) => {
  const pkg = {
    config: {
      target: '1.0.0',
      runtime: 'electron',
